...
\begin{itemize}
  ...
  \item $\rho_k$ The propability that a node with degree k is vulerable (according to a propability distribution)
  \item $p_k$ The propability that a node has the threshold $k$ (according to a propability distribution)
\end{itemize}

In summary, for given propability distributions for both node degree and node threshold, the cascade condition inequation tells us if a vulnerable cluster percolates the network, meaning that global cascades are possible.

To illustrate this, let's choose those distributions and see what happens. We'll construct a uniform graph with a homogenous threshold. Uniform graphs have a poisson degree distribution $p_k = \frac{e^{-z}z^k}{k!}$ with $z$ being the mean (the peak) of the distribution. The fixed threshold is easily modeled: The propability for a node being vulnerable is $\rho_k = P(\phi \leq \frac{1}{k})$, but since $\phi$ (the threshold) in this case is fixed, we just need to use $1$ for $\phi \leq \frac{1}{k}$ and $0$ for $\phi > \frac{1}{k}$ (and $1$ for $k = 0$).

For instance, let's see if cascades are possible for a fixed threshold $\phi = 0.2$ and a poisson-distributed degree with mean $z = 2$:
...
To put this in contrast, let's examine the case of $\phi = 0.2$ and $z = 10$:
